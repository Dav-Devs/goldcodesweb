<nav class="navbar navbar-expand-md navbar-dark bg-dark" style="background: #b1871b !important" role="navigation" aria-label="navbar">
    <div class="container-fluid">
      <a class="navbar-brand" href="/">
              @yield('logo_svg')
              {{ str_replace('_', ' ', config('app.name', 'Gold_Codes_Web')) }}
      </a>
      <button class="navbar-toggler collapsed border-0" type="button" data-bs-toggle="collapse" data-bs-target="#b_navbar" aria-controls="b_navbar" aria-expanded="false" aria-label="Toggle navigation">
...
@guest
              ...
              <li class="nav-item">
                  <a class="nav-link btn btn-sm btn-outline-warning rounded-25 r_btn" href="{{ route('register') }}">{{ __('Register') }}
                      <span>&raquo;</span></a>
              </li>
          </ul>
...
@else
          <ul class="navbar-nav dropdown">
            <a class="nav-link dropdown-toggle" style="cursor: pointer;" {{-- href="#" --}} id="dropdown04" data-bs-toggle="dropdown" aria-expanded="false">
                <img src="{{ URL::signedRoute('icon.png', ['s' => 100, 'uid' => md5(Auth::user()->email)]); }}" width="26" height="26" 
                alt="{{ Auth::user()->name }}" title="{{ Auth::user()->name }}" />
            </a>
            <ul class="dropdown-menu dropdown-menu-dark border-0 rounded-0" {{-- style="background: #fcf6e9" --}} aria-labelledby="dropdown04">
